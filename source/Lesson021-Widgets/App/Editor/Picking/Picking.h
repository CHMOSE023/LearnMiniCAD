...
{ 
    ...
    public:
		...
		bool ConsumeSelectionChanged(); // 供外层查询选集变化并重置状态（主要是 GripManager）
    ...
    private:
		...
		bool m_selectionDirty = false;                        //  选择集是否发生变化（用于通知 GripManager 刷新夹点）

		std::unordered_set<Object::ObjectID> m_prevSelection; // 上一次框选的结果（仅 BoxSelecting 有效，用于增量更新）
