...
namespace MiniCAD
{
    ...
    void SceneExtractor::Extract(const ISceneReader& sceneReader, std::vector<Vertex_P3_C4>& verteies)
    {
        verteies.clear(); 

        ExtractSceneVerteies(sceneReader, verteies);
         
    }

    void SceneExtractor::ExtractSceneVerteies(const ISceneReader& sceneReader, std::vector<Vertex_P3_C4>& items)
    {
        sceneReader.ForEachObject([&](const Object& obj)
        {  
				// 这里只处理 LineEntity，其他类型（CircleEntity、PreviewEntity）可以类似处理
