...
{  
    ...
    static GripGeometry BuildGripGeometry(const RenderTarget& target, const ViewState& vs)
    {
        GripGeometry g;
    ...
    static void AddDashedLine(std::vector<Vertex_P3_C4>& out, XMFLOAT3& a, XMFLOAT3& b, XMFLOAT4& color, float dashLen = 6.0f, float gapLen = 4.0f)
    {
        using namespace DirectX;
    ...
    static SelectionGeometry BuildSelectionGeometry(const RenderTarget& target, const ViewState& sel)
    {
        SelectionGeometry g;

        int x0 = sel.Selection.Start.x;
        int y0 = sel.Selection.Start.y;
        int x1 = sel.Selection.End.x;
        int y1 = sel.Selection.End.y;
    ...
    {   
        ...
        if (viewState.ShowGizmo && viewState.Grips.size() > 0)
        {
            auto grips = BuildGripGeometry(target, viewState);

            m_renderer.Submit(grips.fill,   grips.screenVP, PrimitiveType::Triangle, true, true);
            m_renderer.Submit(grips.border, grips.screenVP, PrimitiveType::Line, true, true);
        }
