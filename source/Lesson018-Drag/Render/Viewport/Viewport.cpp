...
{    
    ...
    void Viewport::Render(const RenderTarget& target, const ViewState& viewState)
    {   
        XMMATRIX vp   = m_camera.GetViewProj();        
        auto screenVP = XMMatrixOrthographicOffCenterLH(0.0f, target.viewport.Width, target.viewport.Height, 0.0f, 0.0f, 1.0f);
         
        m_renderer.BeginFrame(target); 
        {  
           
            { // 轴网 
                auto grid = m_grid.BuildGrid(m_camera, viewState.ShowGrid, target.viewport.Width, target.viewport.Height);
                if (!grid.empty())
                    m_renderer.Submit(grid, screenVP, PrimitiveType::Line, true, true);
            }

            { // 鼠标样式
                auto cursor = m_cursor.BuildCursor(viewState, target.viewport.Width, target.viewport.Height);
                m_renderer.Submit(cursor, screenVP, PrimitiveType::Line, true, true);

            }

            { // 最近点
                auto snop = BuildSnapGeometry(target, viewState); 
            ...
            { // 渲染夹点
                ...
                {
                    auto grips = BuildGripGeometry(target, viewState);
    ...
    // 构建选择框
    SelectionGeometry Viewport::BuildSelectionGeometry(const RenderTarget& target, const ViewState& vs)
    {
        SelectionGeometry g;
    ...
    GripGeometry Viewport::BuildGripGeometry(const RenderTarget& target, const ViewState& vs)
    {
        GripGeometry g;
    ...
    SnapGeometry Viewport::BuildSnapGeometry(const RenderTarget& target, const ViewState& state)
    {
