...
{ 
    ...
    void Viewport::Render(const RenderTarget& target, 
                          const std::vector<Vertex_P3_C4>& sceneVertices,  
                          const std::vector<Vertex_P3_C4>& overlayVertices) 
    {  
        XMMATRIX vp = m_camera.GetViewProj();
         
        m_renderer.Begin(target);                    // BeginFrame
         
        m_renderer.DrawLine(sceneVertices, vp);      // 场景内容

        m_renderer.DrawLine(overlayVertices, vp);    // 绘制预览
